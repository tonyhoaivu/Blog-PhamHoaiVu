...
const SidebarLogin: React.FC<SidebarLoginProps> = ({ currentUser, onLogin, onLogout, navigateTo }) => {
  ...
  // Load tài khoản đã ghi nhớ từ LocalStorage khi khởi tạo
  useEffect(() => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Sử dụng mật khẩu chính xác từ yêu cầu người dùng
    const ADMIN_USER = "admin";
    const ADMIN_PASSWORD = "092709940@Phv"; 
    ...
    return (
      <div className="bg-white rounded-[2rem] p-8 shadow-xl border border-sky-50 animate-in zoom-in duration-300">
        <div className="flex items-center gap-4 mb-6">
          <div className="w-14 h-14 rounded-2xl bg-sky-600 text-white flex items-center justify-center font-black text-2xl shadow-lg shadow-sky-500/20">
        ...
        <div className="space-y-3">
          {currentUser.role === 'admin' && (
            <button 
              ...
              className="w-full bg-sky-600 text-white py-3.5 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-sky-700 shadow-xl shadow-sky-600/10 transition-all active:scale-95"
            >
              BẢNG ĐIỀU KHIỂN
            </button>
          )}
          <button 
            ...
            className="w-full bg-slate-100 text-slate-500 py-3.5 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-red-50 hover:text-red-600 transition-all"
          >
            ĐĂNG XUẤT
          </button>
        </div>
    ...
    <div className="bg-white rounded-[2rem] p-8 shadow-xl border border-sky-50 animate-in fade-in duration-300">
      ...
      <div className="space-y-3 mb-6">
        <button 
          ...
          disabled={isLoading}
          className="w-full flex items-center justify-center gap-3 py-3 border border-slate-200 rounded-xl hover:bg-slate-50 transition shadow-sm group"
        >
        ...
        <button 
          ...
          disabled={isLoading}
          className="w-full flex items-center justify-center gap-3 py-3 bg-[#1877F2] text-white rounded-xl hover:bg-blue-700 transition shadow-sm group"
        >
      ...
      <div className="relative flex items-center gap-4 mb-6 opacity-30">
        ...
        <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">Admin Login</span>
        <div className="flex-grow h-px bg-slate-400"></div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-3">
          ...
          <div className="relative">
            ...
            <button 
              ...
              className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-sky-600 text-[10px] font-black"
            >
        ...
        <div className="flex items-center gap-2 px-1">
          ...
          />
          <label htmlFor="remember_quick" className="text-[10px] font-black text-slate-400 uppercase tracking-tighter cursor-pointer select-none">Ghi nhớ đăng nhập</label>
        </div>

        <button 
          type="submit"
          disabled={isLoading}
          className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-black shadow-xl transition-all active:scale-95 disabled:bg-slate-400"
        >
